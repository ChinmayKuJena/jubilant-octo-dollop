...
export class ImagechatGateway implements OnGatewayInit {
  ...
  handleJoinRoom(
    ...
    // Send a message
    @SubscribeMessage('sendMessageImage')
    async handleMessage(
      @MessageBody() { roomId, message,url }: { roomId: string; message: string ,url:string},
      @ConnectedSocket() client: Socket,
    ) {
      ...
      try {
        ...
        this.server.to(roomId).emit('receiveMessageImage', {
